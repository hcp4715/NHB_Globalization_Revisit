...
```{r CHN Sex ratio and Age, message=FALSE, warning=FALSE}
...
fig1_abc <- fig1a + fig1b + fig1c + plot_annotation(tag_levels = 'A')#  + plot_layout(heights = c(1, 1.3))

ggsave("fig1_abc_CHNData.pdf", fig1_abc, device = "pdf", width=15, height = 9)
...
fig1de <- (fig1e | fig1d) + plot_layout(guides = "keep") #+ plot_annotation(tag_levels = c('D', '1'))

fig1de

ggsave("fig1_de_CHNData.pdf", fig1de, device = "pdf", width=20, height = 15)
```

# Nigeria Data

The data of Nigerial are from different sources. 
...
map_ng(region = df_ng_census2019_geo$state, x = df_ng_census2019_geo$Proportion, breaks = bb, col = 'Blues', show.text = FALSE, leg.title = "Proportions (%)")

df_ng_ruggeri_geo <- dat_ng %>%
  dplyr::filter(!is.na(REGION)) %>%
  ...
  tidyr::complete(state, fill = list(Proportion=0)) %>%
  dplyr::mutate(state = as.character(state))

map_ng(region = df_ng_ruggeri_geo$state, x = df_ng_ruggeri_geo$Proportion, breaks = bb, col = 'Blues', show.text = FALSE, leg.title = "Proportions (%)")
