...
pacman::p_load('tidyverse', 'easystats', 'patchwork', 'here', 'bruceR', "naijR", "wbstats")
...
df_census_us_states <- bruceR::import("DECENNIALPL2020.P1-2023-02-08T080125.csv")

df_census_us_age <- bruceR::import("ACSST5Y2021.S0101-2023-02-08T075713.csv") %>%
  dplyr::select(`Label (Grouping)`, contains("Estimate"))

names <- c("ageBins", "Total", "Total_prop", "Male", "Male_prop", "Female", "Female_prop")
colnames(df_census_us_age) <- names

df_census_us_age <- df_census_us_age %>%
...
## sex ratio
df_census_us_sex <- df_census_us_age %>%
  dplyr::slice(1) %>%
...
## age bins
df_census_us_age_all <- df_census_us_age %>%
  dplyr::slice(3:15, 32) %>%
...
dat_ruggeri_us_age <- dat_ruggeri_us %>%
  dplyr::filter(!is.na(Age)) %>%
    dplyr::mutate(ageBins = cut(Age, 
                                  breaks=c(15, 20, 25, 30, 35, 40, 45,
                                           50, 55, 60, 65, Inf), 
...
ggplot(data = df_census_us_age_all, 
       aes(x=ageBins, y=ifelse(Sex=="Male", -Proportion, Proportion), fill=Sex)) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = dat_ruggeri_us_age, aes(x=ageBins,
                                      y = ifelse(Sex == "Male", -Proportion, Proportion),
                                      group=Sex,color="Ruggeri2022"), size=1, inherit.aes = FALSE) +
...
### education
df_census_us_edu <- bruceR::import("ACSST5Y2021.S1501-2023-02-08T080006.csv")
