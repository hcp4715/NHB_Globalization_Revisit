...
pacman::p_load('tidyverse', 'easystats', 'patchwork', 'here', 'bruceR', "naijR", "tidycensus")
```
...
save(df_cn_census, df_cn_census_geo, file = here::here("Re_Analysis_CHN.RData"))
...
```{r CHN Sex and Age, message=FALSE, warning=FALSE}
...
# df_cn_sex_ratio_jasp <- df_cn_sex_ratio %>%
...
```{r Nigeria data}
## data source
# http://nigeria-education.org/data
...
df_world_1 <- read_csv('Data_extracte_WorldBankData_Series_Data.csv')
df_world_2 <- read_csv('Data_extracte_WorldBankData_Series_Metadata.csv')
  
str(df_world_1)
str(df_world_2)
  
df_ng_2021 <- df_world_1 %>%
...
df_ng_2021_age_final <- df_ng_2021_age %>%
  dplyr::mutate(sex = str_replace_all(sex, "\\s", "")) %>%
  dplyr::mutate(Pop = ifelse(sex == "female", 
                             round(Percent * df_ng_2021_pop$female, 0),
                             round(Percent * df_ng_2021_pop$male,0)),
                Pop_percent = Pop/df_ng_2021_pop$total) %>%
...
df_ng_2021_age_final_1 <- df_ng_2021_age_final %>%
  dplyr::filter(ageBins %in% Age_bins_ng_val[1:10])

df_ng_2021_age_final_2 <- df_ng_2021_age_final %>%
  dplyr::filter(ageBins %in% Age_bins_ng_val[11:14]) %>%
  dplyr::group_by(Sex) %>%
  dplyr::summarise(n_prop = sum(Proportion)) %>%
  dplyr::ungroup() %>%
  dplyr::rename(Proportion = n_prop) %>%
  dplyr::mutate(ageBins = ">=65") %>%
  dplyr::bind_rows(., df_ng_2021_age_final_1) %>%
  dplyr::mutate(ageBins = factor(ageBins, 
                                 levels = Age_bins_ng_final)) %>%
  dplyr::arrange(ageBins)
  

dat_ruggeri_ng <- dat_unique %>%
  dplyr::filter(Residence == "Nigeria") %>%
  dplyr::select(ResponseId, Gender, EducationCompleted, Residence, Age, "Ethnic", "REGION" ) %>%
  dplyr::filter(Gender == "Female" | Gender == "Male")

dat_ruggeri_ng_age <- dat_ruggeri_ng %>%
  dplyr::filter(!is.na(Age)) %>%
    dplyr::mutate(ageBins = cut(Age, 
...
ggplot(data = df_ng_2021_age_final_2, 
       aes(x=ageBins, y=ifelse(Sex=="Male", -Proportion, Proportion), fill=Sex)) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = dat_ruggeri_ng_age, aes(x=ageBins,
                                      y = ifelse(Sex == "Male", -Proportion, Proportion),
                                      group=Sex,color="Ruggeri2022"), size=1, inherit.aes = FALSE) +
...
df_ng_reg_2019 <- bruceR::import("NG_pop_2019_wiki.csv")
...
bb <- c(0, 0.001, 0.5, 5, 10, 20, 50, 100)
color_val <- c("#FFFFFF","#9ECAE1","#6BAED6", # , "#C6DBEF"
                               "#4292C6", "#2171B5", "#08519C","#08306B")

map_ng(region = df_ng_reg_2019$state, x = df_ng_pop_2019$Proportion, breaks = bb, col = 'Blues', show.text = FALSE, leg.title = "Proportions (%)")


df_ng_reg_Ruggeri2022 <- dat_ng %>%
  dplyr::filter(!is.na(REGION)) %>%
...
df_ng_reg_Ruggeri2022_prop <- df_ng_reg_Ruggeri2022 %>%
...
df_tmp <- data.frame(state = naijR::states(),
                     state_n = 0,
                     Proportion = 0) %>%
  dplyr::filter(!(state %in% df_ng_reg_Ruggeri2022_prop$state))

df_ng_reg_Ruggeri2022_prop <- rbind(df_ng_reg_Ruggeri2022_prop, df_tmp) %>%
  dplyr::arrange(state)


map_ng(region = df_ng_reg_Ruggeri2022_prop$state, x = df_ng_reg_Ruggeri2022_prop$Proportion, breaks = bb, col = 'Blues', show.text = FALSE, leg.title = "Proportions (%)")
...
p_reg_ng_ruggeri2022 <- map_ng(region = states(), x = nn, breaks = bb, col = 'YlOrRd', show.text = FALSE)

library(patchwork)

p_reg_ng <- (p_reg_ng_wiki2019 + p_reg_ng_ruggeri2022)

ng_df <- readr::read_csv("nga_general_2020_csv.zip")
```
...
ng_edu_prop <- data.frame(Female = c(35, 14, 16, 23, 12), # originally sum up to 99, randomly
                          Male = c(22, 14, 16, 32, 16), # originally sum up to 101
                          Edu = c("NoSchool", "Primary", "Some_secondary", 
                                   "Completed_secondary", "Tertiary")) %>%
  dplyr::mutate(Female = Female/2,
                Male = Male/2) %>%
  tidyr::pivot_longer(cols = Female:Male,names_to = "Sex", values_to = "Proportion") %>%
  dplyr::mutate(Edu = factor(Edu, levels = c("NoSchool", "Primary", "Some_secondary", "Completed_secondary",
                                        "Tertiary")))
...
dat_ng_edu$Edu <- dplyr::recode(dat_ng_edu$EducationCompleted,
                                Graduate = "Tertiary",
                                MBA = "Tertiary",
                                `Primary ed.` = "Primary",
                                `Secondary ed.` = "Completed_secondary",
                                `Technical ed.` = "Tertiary",
                                `No formal ed.` = "NoSchool",
                                Bachelor = "Tertiary",
                                ) %>%
  factor(., levels = c("NoSchool", "Primary", "Some_secondary", "Completed_secondary",
                                        "Tertiary")) 
  
dat_ng_edu_prop <- dat_ng_edu %>%
...
ggplot(data= ng_edu_prop, 
       aes(x=Edu, y=ifelse(Sex=="Male", -Proportion, Proportion), fill=Sex)) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = dat_ng_edu_prop, 
            aes(x=Edu,
                y = ifelse(Sex == "Male", -Proportion, Proportion),
                group=Sex, color="Ruggeri2022"), size=1, inherit.aes = FALSE) +
  # scale_y_continuous(limits = c(-15,15), sec.axis = sec_axis(~.*1, name = "Proportion (Ruggeri2022)")) +
...
# # install the api key and activate it
# census_api_key("321ed99e90327e7474975d8e95427fcc69bb2bba", install = TRUE)
# readRenviron("~/.Renviron")
# 
# df_census_us <- tidycensus::get_decennial(
#   geography = "state",
#   year = 2020,
#   sumfile = "pl"
#   
...
names <- c("ageBins", "Total", "Total_prop", "Male", "Male_prop", "Female", "Female_prop")
colnames(df_us_census_age) <- names

df_us_census_age <- df_us_census_age %>%
...
ggplot(df_us_sex_ratio, aes(Site, Proportion,fill=Sex)) +
  geom_col() +
  theme_classic()+
  coord_flip() +
  xlab("Data sources") +
...
ggplot(data = df_us_census_age_all, 
       aes(x=ageBins, y=ifelse(Sex=="Male", -Proportion, Proportion), fill=Sex)) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = df_us_ruggeri2022_age, aes(x=ageBins,
                                      y = ifelse(Sex == "Male", -Proportion, Proportion),
                                      group=Sex,color="Ruggeri2022"), size=1, inherit.aes = FALSE) +
  scale_y_continuous(limits = c(-15, 15)) +
  coord_flip() +
  labs(y="Proportion", x = "Age bins", color=NULL)+
  annotate("text",label= "italic(Male)", x=10,y=-5, parse=TRUE,size=8, family = "serif") +
  annotate("text",label= "italic(Female)", x=10,y=6, parse=TRUE,size=8,family = "serif") +
  scale_color_manual(values = c("red","blue"))+
  theme_classic()+
  theme(panel.border =element_rect(fill=NA,color="black"),
        legend.position ="bottom",
...
df_census_us_edu <- bruceR::import("ACSDT5Y2021.B15001-2023-02-08T135043.csv") %>%
  dplyr::select(`Label (Grouping)`, contains("Estimate")) %>%
...
df_census_us_edu_all <- df_census_us_edu %>%
...
df_ruggeri_us_edu <- df_us_ruggeri2022 %>%
  dplyr::filter(!is.na(EducationCompleted)) %>%
...
df_us_edu <- rbind(df_ruggeri_us_edu, df_census_us_edu_all)

ggplot(df_us_edu, aes(x=Proportion, y=Sex, fill=Edu)) +
  geom_col(alpha = .75)+
  labs(y="Sex") + 
...
ggplot(data= df_census_us_edu_all, 
       aes(x=Edu, y=ifelse(Sex=="Male", -Proportion, Proportion), fill=Sex)) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = df_ruggeri_us_edu, 
            aes(x=Edu,
                y = ifelse(Sex == "Male", -Proportion, Proportion),
                group=Sex, color="Ruggeri2022"), size=1, inherit.aes = FALSE) +
  # scale_y_continuous(limits = c(-15,15), sec.axis = sec_axis(~.*1, name = "Proportion (Ruggeri2022)")) +
...
propBins_us <- c("<=0.49", "0.5-0.99", "1.0-1.49", "1.5-1.99",
              "2-4.99", "5-11.99", ">=12")

df_census_us_geo <- bruceR::import("DECENNIALPL2020.P1-2023-02-08T080125.csv") %>%
  dplyr::slice(1) %>%
  ...
  dplyr::mutate(Proportion = round(Pop / sum(Pop), 4) * 100,
                Source = "Cenesus_2020") %>%
  dplyr::mutate(propBins = cut(Proportion, 
                              breaks=c(0, 0.5, 1, 1.5, 2, 5, 12, 20), 
                              labels=propBins_us),
                propBins = factor(propBins, 
                                  levels = c("0", propBins_us))
                ) %>%
  dplyr::arrange(State)

df_census_us_geo_p <- us_median_age %>%
  dplyr::arrange(NAME) %>%
  dplyr::mutate(Proportion = df_census_us_geo$Proportion,
                propBins = df_census_us_geo$propBins)

df_ruggeri_us_geo <- df_us_ruggeri2022 %>%
  dplyr::filter(!is.na(REGION)) %>%
  dplyr::group_by(REGION) %>%
  dplyr::summarise(N = n()) %>%
  dplyr::ungroup() %>%
  dplyr::filter(REGION %in% df_census_us_geo$State) %>%
  dplyr::mutate(Proportion = round(N / sum(N), 4) * 100,
                Source = "Ruggeri_2022") %>%
  dplyr::mutate(propBins = cut(Proportion, 
                              breaks=c(0, 0.5, 1, 1.5, 2, 5, 12, 20), 
  ...
  dplyr::mutate(REGION = factor(REGION,
                                levels = df_census_us_geo$State)) %>%
  tidyr::complete(REGION, fill = list(Proportion=0, propBins = "0"))
  
df_ruggeri_us_geo_p <- us_median_age %>%
  dplyr::arrange(NAME) %>%
  dplyr::mutate(Proportion = df_ruggeri_us_geo$Proportion,
                propBins = df_ruggeri_us_geo$propBins)

# library(tidycensus)
# library(tidyverse)
# library(tigris)

# census_api_key("321ed99e90327e7474975d8e95427fcc69bb2bba", install = TRUE, overwrite=TRUE)
# readRenviron("~/.Renviron")
# 
...
p_census_us_geo <- ggplot(data = df_census_us_geo_p, aes(fill = propBins)) + 
  geom_sf() + 
...
p_ruggeri_us_geo <- ggplot(data = df_ruggeri_us_geo_p, aes(fill = propBins)) + 
  geom_sf() + 
  scale_fill_manual("Proportion",
                    values = c("#FFFFFF", "#C6DBEF","#9ECAE1","#6BAED6",
                               "#4292C6", "#2171B5", "#08519C","#08306B"),
  ...
  labs(#title = "Distribution of population",
       caption = "Data source: Ruggeri et al 2022 NHB") + 
  theme_void()

p_census_us_geo + p_ruggeri_us_geo
```